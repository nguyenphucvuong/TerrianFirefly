...
import { ButtonsComponent, IconsOptionComponent } from "../";
  ...
  const btnDangComment = async () => {
    ...
    if (content || image) {
      ...
      if (image) {
        ...
        if (
          predictions[3].probability > 0.05 ||
          predictions[2].probability > 0.05 ||
          predictions[4].probability > 0.05
        ) {
          if (Platform.OS === "android") {
      ...
      isSubCmt
        ? [
            dispatch(createSubComment(dataCmt)),
            dispatch(
              createNoti({
                post_id: post.post_id,
                user_id: user_id,
      ...
      if (Platform.OS === "android") {
        ToastAndroid.show(
          "Bình luận hoặc hình không được để trống!",
          ToastAndroid.SHORT
        );
      } else {
